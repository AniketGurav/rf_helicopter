...
trials = 100                                 # Number of Trials to Train the Agent on
r_values = [10, -1, 1]                      # List of Rewards [Completed, Crashed, Open]
p_values = [0.5, 0.5, 0.5] 
d_par = 1
par="none"                                   #epsilon, alhpa, gamma
plt_factor = [0,list(range(trials - 0,trials +1)), 10000]   # Plot first X items
nb_actions = 3                                  # Number of Possible Actions Helicopter can take
model = [1,2]                                     # Model Selection
decay_rate = 0.9                                # Learning Rate Decay (if model=2)
nb_action_change = 6000                         # Learning Rate Decay update rate (if model=2)
lmb = 2                                         # Temporal Difference value (if model=3)
color=["red","blue","yellow"]
moddef=["Monte Carlo", "Epsylon Decay", "Temporal Difference"]
...
for z in model:  
    ...
    fig = plt.figure()    
    fig.canvas.draw()
    fig.canvas.set_window_title(moddef[z-1])
    plt.subplot(2,2,1)
    ...
    for aa in range(1,d_par+1):
        ...
        while HeliWorld.trials <= trials:
            if HeliWorld.trials % plt_factor[2] == 0 and HeliWorld.trials > 0:
                ...
                logging.info("Trials Completed: {} at {:.4f} Trails / seconds".format(value[0],value[1]))
            ...
            while True:
                ...
                if not output:
                    ...
                    plt.scatter(HeliWorld.trials,
                                Helicopter1.final_location[-1][0]/float(HeliWorld.track_width),
                                s=np.pi * (1 * (1))**2,
                                c=colors[aa-1],
                                alpha=0.5)
                ...
                if HeliWorld.trials <= plt_factor[0] or HeliWorld.trials in plt_factor[1]:
                    # Primary Title
                    rate = (time() - st + 0.01) / HeliWorld.trials
                    value = [HeliWorld.trials,
                             rate]
                    fig.suptitle(
                        'Time for Trial Completion: {} - \
                                  ...
                                  Agent Model: {} \n\
                                  Agent Parameters: \
                                  alpha {:.4f} - epsilon {:.4f} - gamma {:.4f} - Number of Actions: {}\n\
                                  World Paramers: \
                ...
                view_current = Helicopter1.q_matrix[Helicopter1.current_location[0]-1][0]
                qw_mat = []
                for i in range(5):
                ...
                if len(bigger_array[0,start-2:start+3]) <5:
                    bigger_array[0,max(start-2,0):max(start+3,5)] = masked_smaller_array 
                else:
